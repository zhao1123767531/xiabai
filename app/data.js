/* 瞎掰王 — 数据：生僻词库 + 昵称 + 头像配色 + 自定义词库存储 */
(function () {
  const DIRECTION_POOL = ["成语", "地方", "小说", "植物", "神兽", "服饰", "器物", "天象", "水文", "民俗", "动作", "状态", "言论", "宴饮", "工艺", "昆虫"];
  function withDirs(word) {
    const answer = word.answer || (word.dirs && word.dirs[0]) || "词语";
    const dirs = [...(word.dirs || [answer])].filter(Boolean);
    if (!dirs.includes(answer)) dirs.unshift(answer);
    DIRECTION_POOL.forEach((d) => { if (dirs.length < 3 && !dirs.includes(d)) dirs.push(d); });
    return { ...word, answer, dirs: dirs.slice(0, 3) };
  }

  // 默认生僻词（含拼音、释义与三个方向）—— 老实人可见真释义和正确方向
  window.XBW_DEFAULT_WORDS = [
  ...
  const KEY = "xbw_custom_words";
  function loadCustom() {
    try { const a = JSON.parse(localStorage.getItem(KEY) || "[]"); return Array.isArray(a) ? a : []; }
    catch (e) { return []; }
  }
  // 当前生效词库：有自定义则用自定义，否则用默认
  window.getActiveWords = function () {
    const c = loadCustom();
    return (c.length ? c : window.XBW_DEFAULT_WORDS).map(withDirs);
  };
  window.getCustomWords = loadCustom;
  window.isUsingCustom = function () { return loadCustom().length > 0; };
  window.saveCustomWords = function (arr) { localStorage.setItem(KEY, JSON.stringify(arr)); };
  window.clearCustomWords = function () { localStorage.removeItem(KEY); };
  // 兼容旧引用
  window.XBW_WORDS = window.getActiveWords();

  // 解析导入文本：优先按 | 分隔，避免释义里的逗号被误切。
  window.parseWordLib = function (text) {
    const words = [], errors = [];
    (text || "").split(/\r?\n/).forEach((raw, i) => {
      const line = raw.trim();
      if (!line) return;
      const splitter = /[|｜]/.test(line) ? /\s*[|｜]\s*/ : /\s*[\t,，]\s*/;
      const parts = line.split(splitter).map((s) => s.trim()).filter(Boolean);
      if (parts.length === 1) { errors.push("第" + (i + 1) + "行：缺少释义 → " + line); return; }
      let w, py = "", def, answer = "", dirs = [];
      if (parts.length === 2) { w = parts[0]; def = parts[1]; }
      else {
        w = parts[0]; py = parts[1]; def = parts[2];
        if (parts.length >= 5) {
          answer = parts[3];
          dirs = [answer, ...parts.slice(4).join("/").split(/[\/、]/).map((s) => s.trim()).filter(Boolean)];
        } else {
          def = parts.slice(2).join("，");
        }
      }
      if (!w || !def) { errors.push("第" + (i + 1) + "行：格式不正确 → " + line); return; }
      words.push(withDirs({ w, py, def, answer, dirs }));
    });
    return { words, errors };
  };
